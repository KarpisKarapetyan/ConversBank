import banner1 from '../Components/HomePage/ConversPoints/PointsSlider/img/banner1.jpg'
import banner2 from '../Components/HomePage/ConversPoints/PointsSlider/img/banner2.jpg'
import banner3 from '../Components/HomePage/ConversPoints/PointsSlider/img/banner3.jpg'

export const symbolRate = ["֏" , ' ' , ' ']
export const linksArr = {
        headerManuTitls : [
            [
                {
                    name : 'Վարկեր',
                    link : '/'
                },
                {
                    name : 'Ավանդներ',
                    link : '/'
                },
                {
                    name : 'Քարտեր',
                    link : '/'
                },
                {
                    name : 'Հաշիվներ',
                    link : '/'
                },
                {
                    name : 'Փոխանցումներ',
                    link : '/'
                },
                {
                    name : 'Ներդրումներ',
                    link : '/'
                },
                {
                    name : 'Այլ ծառայություններ',
                    link : '/'
                }
            ],
            [
                {
                    name : 'Кредиты',
                    link : '/'
                },
                {
                    name : 'Депозиты',
                    link : '/'
                },
                {
                    name : 'Карты',
                    link : '/'
                },
                {
                    name : 'Cчета',
                    link : '/'
                },
                {
                    name : 'Трансферы',
                    link : '/'
                },
                {
                    name : 'Инвестиции',
                    link : '/'
                },
                {
                    name : 'Другие услуги',
                    link : '/'
                }
            ],
            [
                {
                    name : 'Loans',
                    link : '/'
                },
                {
                    name : 'Deposits',
                    link : '/'
                },
                {
                    name : 'Cards',
                    link : '/'
                },
                {
                    name : 'Invoices',
                    link : '/'
                },
                {
                    name : 'Transfers',
                    link : '/'
                },
                {
                    name : 'Investments',
                    link : '/'
                },
                {
                    name : 'Other services',
                    link : '/'
                }
            ],
            
            ],
            headerManuChildDrops : [
                [[
                    {
                        name : "Հիփոթեք",
                        link : '/'
                    },
                    {
                        name : "Սպառողական վարկեր",
                        link : '/'
                    },
                    {
                        name : "Գյուղատնտեսական վարկեր",
                        link : '/'
                    },
                    ],
                    [
                        {
                            name : "Օրական ",
                            link : '/'
                        },
                        {
                            name : "Գտնված երազ",
                            link : '/'
                        },
                        {
                            name : "Կուտակային \n կենսաթոշակային ավանդ ",
                            link : '/'
                        },
                        {
                            name : "Կոնվերս 25 ",
                            link : '/'
                        },
                        {
                            name : "Պրոգրես",
                            link : '/'
                        },
                        {
                            name : "Կարաս",
                            link : '/'
                        },
                        {
                            name : "Մանուկ ",
                            link : '/'
                        },
                        {
                            name : "Բարեկեցիկ ապագա",
                            link : '/'
                        }],
                        [
                            {
                                name : "Հատուկ քարտեր",
                                link : '/'
                            },
                            {
                                name : "Ստանդարտ",
                                link : '/'
                            },
                            {
                                name : "Պրեմիում",
                                link : '/'
                            },
                            {
                                name : "Contactless լուծումներ",
                                link : '/'
                            }],
                            [
                                {
                                    name : "Էսքրոու հաշիվներ",
                                    link : '/'
                                },
                                {
                                    name : "Սնանկություն",
                                    link : '/'
                                },
                                {
                                    name : "Սոցիալական փաթեթներ",
                                    link : '/'
                                },
                                {
                                    name : "Պետական աջակցության հաշիվ",
                                    link : '/'
                                },
                                {
                                    name : "Այլ հատուկ հաշիվներ",
                                    link : '/'
                                }],
                                [
                                    {
                                        name : "5rd LinksAM",
                                        link : '/'
                                    },
                                    {
                                        name : "5rd LinksAM",
                                        link : '/'
                                    },
                                    {
                                        name : "5rd LinksAM",
                                        link : '/'
                                    },
                                    {
                                        name : "5rd LinksAM",
                                        link : '/'
                                    }],
                                    [
                                        {
                                            name : "Ներդրումային գործունեություն",
                                            link : '/'
                                        },
                                        {
                                            name : "Շուկա ստեղծող (Market maker)",
                                            link : '/'
                                        },
                                        {
                                            name : "Պարտատոմս",
                                            link : '/'
                                        },
                                        {
                                            name : "Պահառուական ծառայություններ",
                                            link : '/'
                                        }],
                                        [
                                            {
                                                name : "Գործարքներ",
                                                link : '/'
                                            },
                                            {
                                                name : "ԿԲՀՏ/ԿԲԼՏ",
                                                link : '/'
                                            },
                                            {
                                                name : "Ապահովագրություն",
                                                link : '/'
                                            },
                                            {
                                                name : "Պետ մարմինների սպասարկման գրասենյակներ",
                                                link : '/'
                                            },{
                                                name : "Ծառայությունների ուղղորդում",
                                                link : '/'
                                            },
                                            {
                                                name : "Տարադրամի և ոսկու",
                                                link : '/'
                                            },
                                            {
                                                name : "Գնահատող ընկերություններ",
                                                link : '/'
                                            },
                                            {
                                                name : "Այլ սակագներս",
                                                link : '/'
                                            }]
                       
                       ],
                       [[
                        {
                            name : "Ипотека",
                            link : '/'
                        },
                        {
                            name : "Потребительский кредит",
                            link : '/'
                        },
                        {
                            name : "Сельскохозяйственные кредиты",
                            link : '/'
                        }],
                        [
                            {
                                name : "Повседневная",
                                link : '/'
                            },
                            {
                                name : "Найденная мечта",
                                link : '/'
                            },
                            {
                                name : "Накопительный \n пенсионный вклад",
                                link : '/'
                            },
                            {
                                name : "Конверс",
                                link : '/'
                            },
                            {
                                name : "прогресс",
                                link : '/'
                            },
                            {
                                name : "Горшок",
                                link : '/'
                            },
                            {
                                name : "Малыш",
                                link : '/'
                            },
                            {
                                name : "Благополучное будущее",
                                link : '/'
                            }],
                            [
                                {
                                    name : "Специальные карты",
                                    link : '/'
                                },
                                {
                                    name : "Стандарт",
                                    link : '/'
                                },
                                {
                                    name : "Премиум",
                                    link : '/'
                                },
                                {
                                    name : "Contactless Решения",
                                    link : '/'
                                }],
                                [
                                    {
                                        name : "Депозитные счета",
                                        link : '/'
                                    },
                                    {
                                        name : "Банкротство",
                                        link : '/'
                                    },
                                    {
                                        name : "Социальные пакеты",
                                        link : '/'
                                    },
                                    {
                                        name : "Счет Государственной поддержки",
                                        link : '/'
                                    },
                                    {
                                        name : "Другие специальные счета",
                                        link : '/'
                                    }],
                                    [
                                        {
                                            name : "5rd LinksRU",
                                            link : '/'
                                        },
                                        {
                                            name : "5rd LinksRU",
                                            link : '/'
                                        },
                                        {
                                            name : "5rd LinksRU",
                                            link : '/'
                                        },
                                        {
                                            name : "5rd LinksRU",
                                            link : '/'
                                        }],
                                        [
                                            {
                                                name : "6rd LinksRU",
                                                link : '/'
                                            },
                                            {
                                                name : "6rd LinksRU",
                                                link : '/'
                                            },
                                            {
                                                name : "6rd LinksRU",
                                                link : '/'
                                            },
                                            {
                                                name : "6rd LinksRU",
                                                link : '/'
                                            }],
                                            [
                                                {
                                                    name : "7rd LinksRU",
                                                    link : '/'
                                                },
                                                {
                                                    name : "7rd LinksRU",
                                                    link : '/'
                                                },
                                                {
                                                    name : "7rd LinksRU",
                                                    link : '/'
                                                },
                                                {
                                                    name : "7rd LinksRU",
                                                    link : '/'
                                                }]
                           
                           ],
                           [[
                            {
                                name : "Mortgage",
                                link : '/'
                            },
                            {
                                name : "Consumer credit",
                                link : '/'
                            },
                            {
                                name : "Agricultural loans",
                                link : '/'
                            }],
                            [
                                {
                                    name : "Daily",
                                    link : '/'
                                },
                                {
                                    name : "Dream found",
                                    link : '/'
                                },
                                {
                                    name : "Accumulative \n pension deposit",
                                    link : '/'
                                },
                                {
                                    name : "Convers 25",
                                    link : '/'
                                },
                                {
                                    name : "Progress",
                                    link : '/'
                                },
                                {
                                    name : "Pot",
                                    link : '/'
                                },
                                {
                                    name : "Baby",
                                    link : '/'
                                },
                                {
                                    name : "A prosperous future",
                                    link : '/'
                                },],
                                [
                                    {
                                        name : "Special cards",
                                        link : '/'
                                    },
                                    {
                                        name : "Standard",
                                        link : '/'
                                    },
                                    {
                                        name : "Premium",
                                        link : '/'
                                    },
                                    {
                                        name : "Contactless solutions",
                                        link : '/'
                                    }],
                                    [
                                        {
                                            name : "4rd LinksEN",
                                            link : '/'
                                        },
                                        {
                                            name : "4rd LinksEN",
                                            link : '/'
                                        },
                                        {
                                            name : "4rd LinksEN",
                                            link : '/'
                                        },
                                        {
                                            name : "4rd LinksEN",
                                            link : '/'
                                        }],
                                        [
                                            {
                                                name : "5rd LinksEN",
                                                link : '/'
                                            },
                                            {
                                                name : "5rd LinksEN",
                                                link : '/'
                                            },
                                            {
                                                name : "5rd LinksEN",
                                                link : '/'
                                            },
                                            {
                                                name : "5rd LinksEN",
                                                link : '/'
                                            }],
                                            [
                                                {
                                                    name : "6rd LinksEN",
                                                    link : '/'
                                                },
                                                {
                                                    name : "6rd LinksEN",
                                                    link : '/'
                                                },
                                                {
                                                    name : "6rd LinksEN",
                                                    link : '/'
                                                },
                                                {
                                                    name : "6rd LinksEN",
                                                    link : '/'
                                                }],
                                                [
                                                    {
                                                        name : "7rd LinksEN",
                                                        link : '/'
                                                    },
                                                    {
                                                        name : "7rd LinksEN",
                                                        link : '/'
                                                    },
                                                    {
                                                        name : "7rd LinksEN",
                                                        link : '/'
                                                    },
                                                    {
                                                        name : "7rd LinksEN",
                                                        link : '/'
                                                    }]
                               
                               ]
            ] 
} 
    export const bannerInfo = [
        [
            {
                title : `Անվճար քարտ նոր և \n գործող ավանդների \n համար`,
                message : 'Եթե ունես գործող ավանդ կամ ուզում ես նորը ներդնել, ապա \n այս առաջարկը հենց քեզ համար է',
                img : banner1,
                link : '/'
            },
            {
                title : 'Անվճար քարտ',
                message : 'Եթե ունես գործող',
                img : banner2,
                link : '/'
            },
            {
                title : 'Գործող ավանդ',
                message : 'Ուզու՞մ ես նորը ներդնել',
                img : banner3,
                link : '/'
            },
        ],
        [
            {
                title : ` Безплатная карта для новых и \n существующих депозитов`,
                message : 'Если у вас есть действующий депозит или вы хотите вложить новый, то \n это предложение именно для вас',
                img : banner1,
                link : '/'
            },
            {
                title : 'Безплатная карта',
                message : 'Если у вас есть',
                img : banner2,
                link : '/'
            },
            {
                title : 'Действующий депозит',
                message : 'Вы хотите вложить новый',
                img : banner3,
                link : '/'
            },
        ],
        [
            {
                title : ` Free card for new \n and existing deposits`,
                message : 'If you have an active deposit or want to invest a new one, then  \n this offer is for you',
                img : banner1,
                link : '/'
            },
            {
                title : 'Free card',
                message : 'If you have',
                img : banner2,
                link : '/'
            },
            {
                title : 'Active deposit',
                message : 'You want to invest a new one',
                img : banner3,
                link : '/'
            },
        ]
    ]
    export const footerLinks = [
        {
            title : 'Մեր մասին',
            link : '/'
        },
        {
            title : 'Բանկի մասին',
            link : '/'
        },
        {
            title : 'Բանկի պատմությունը',
            link : '/'
        },
        {
            title : 'Կառուցվածքը',
            link : '/'
        },
        {
            title : 'Նշանակալից մասնակիցներ',
            link : '/'
        },
        {
            title : 'Ղեկավարներ',
            link : '/'
        },
        {
            title : 'Բաժնետերեր և ներդրողներ',
            link : '/'
        },
        {
            title : 'Աշխատանք',
            link : '/'
        },
        {
            title : 'Նարություններ',
            link : '/'
        },
        {
            title : 'Հետադարձ կապ',
            link : '/'
        },
        {
            title : 'Սպասարկման ցանց',
            link : '/'
        },
        {
            title : 'Օգնություն',
            link : '/'
        },
        {
            title : 'Գործընկերներին և մատակարարներին',
            link : '/'
        },
        {
            title : 'Օտարվող գույք',
            link : '/'
        },
        {
            title : 'Հաշվետվություններ',
            link : '/'
        },
        {
            title : 'Իրավական Ակտեր',
            link : '/'
        },
        {
            title : 'Հաճախորդների իրավունքներ',
            link : '/'
        },
        {
            title : 'Ֆինանսական համակարգի հաշտարար',
            link : '/'
        },
        {
            title : 'Մատուցվող ծառայություններ',
            link : '/'
        },
        {
            title : 'ԿԲՀՏ/ Հաշվարկային տոկոսադրույք',
            link : '/'
        },
    ]
    export const linerDots = [
        "Խնայիր տերմինալով վճարելիս ",
        "Mobile Banking experienced in a new thrilling ways",
        "Short Title example",
    ]